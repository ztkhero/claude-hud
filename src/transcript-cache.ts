...
type SerializedTool = Omit<ToolEntry, 'startTime' | 'endTime'> & {
  startTime: string;
  endTime?: string;
};

type SerializedAgent = Omit<AgentEntry, 'startTime' | 'endTime'> & {
  startTime: string;
  endTime?: string;
};
 ...
 */
export function trimParseState(state: ParseState): void {
 ...
 */
async function fingerprint(handle: fs.promises.FileHandle, offset: number): Promise<string> {
  if (offset <= 0) return '';
  ...
  for (let i = 0; i < FINGERPRINT_WINDOWS; i++) {
    ...
    const start = Math.min(
      Math.floor((offset * i) / FINGERPRINT_WINDOWS),
      offset - window
    );
 ...
 */
export async function findLineBoundary(
...
export async function readCache(
  ...
  stat: fs.Stats,
  handle: fs.promises.FileHandle
): Promise<{ state: ParseState; offset: number } | null> {
  if (!shouldCache(stat.size)) return null;

  try {
    const raw = await fs.promises.readFile(getCachePath(transcriptPath), 'utf-8');
    const entry = JSON.parse(raw) as SerializedState;

    if (entry.version !== CACHE_VERSION) return null;
    if (!Number.isInteger(entry.offset) || entry.offset < 0) return null;
    if (stat.size < entry.offset) return null;          // truncated or replaced
    if (stat.mtimeMs < entry.mtimeMs) return null;      // rewound behind us
    if (await fingerprint(handle, entry.offset) !== entry.fingerprint) return null;
...
export async function writeCache(
  ...
  if (!shouldCache(stat.size)) return;

  try {
    const cacheDir = getCacheDir();
    await fs.promises.mkdir(cacheDir, { recursive: true });

    const entry: SerializedState = {
    ...
    if (prune) await pruneCacheDir(cacheDir);
  } catch {
...
async function pruneCacheDir(cacheDir: string): Promise<void> {
  try {
    ...
    await Promise.all(
      stats.slice(0, stats.length - PRUNE_TARGET_FILES)
