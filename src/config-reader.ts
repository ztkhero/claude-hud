...
function readAutoCompactWindow(filePath: string): number | null {
  ...
  try {
    ...
    return (
      parsePositiveNumber(config.env?.[AUTO_COMPACT_WINDOW_ENV]) ??
 ...
 */
export function getAutoCompactWindow(cwd?: string): number | null {
